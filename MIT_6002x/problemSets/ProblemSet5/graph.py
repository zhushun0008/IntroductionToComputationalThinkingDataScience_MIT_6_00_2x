...
class WeightedDigraph(Digraph):
    ...
    def getedge(self, src, dest):
        childList = self.edges[src]

        for i in range(len(childList)):
            if str(childList[i][0]) == str(dest):
                [totalDistance, totalOutdoors] = [int(childList[i][1][0]),
                                                  int(childList[i][1][1])]
                return [totalDistance, totalOutdoors]
    def childrenOf(self, node):
        tempEdges = []
        edgeList = self.edges[node]
        for i in range(len(edgeList)):
            tempEdges.append(edgeList[i][0])
        return tempEdges
    def __str__(self):
        res = ''
        for k in self.edges:
